...
- [x] Milestone 3 (2026-08-21, executor): the Karte screen's Karte/Stempel segmented control
      ...
      requirement.
- [x] Milestone 4 (2026-08-21, executor): documentation reconciled (`app/DESIGN.md`,
      `ARCHITECTURE.md`, `docs/app.md`) and the full acceptance sweep run. Root `npm test` 2
      passed; `app/npm test` 3 suites / 10 tests passed; lint 0 errors + 1 pre-existing warning;
      `tsc --noEmit` 1 pre-existing error, unchanged. `grep -n "rounded-full" app/DESIGN.md` has no
      hit in the Tab bar paragraph; `grep -rn "TabNavigator\|navigation" ARCHITECTURE.md
      docs/app.md` hits in both files.
- [x] ExecPlan finalized (2026-08-21, executor): Outcomes & Retrospective written, plan moved from
...
- Decision: the Code-Quality Gate was **skipped for Milestone 4**. Rationale: Milestone 4 touched
  only `app/DESIGN.md`, `ARCHITECTURE.md`, and `docs/app.md` — no production code, no test, no
  config file — so the executor's own skip condition ("doc-only or config-only changes with no
  logic") applies. All three diffs were reviewed manually by the executor against the plan's exact
  edit instructions before committing (see the `git diff` transcript captured during this
  milestone).
  Date/Author: 2026-08-21, executor.
...
`3985895` tab shell, `b55b3da` segmented control, plus this documentation commit). The app now
builds, from `App.tsx` down, exactly the shape the Purpose section described: a hand-rolled
`TabNavigator` holding a single `useState<TabId>`, a presentational `TabBar` reading vendored
Lucide-style SVG icons and the light design-token palette, three always-mounted screens, and a
Karte screen with a real Karte/Stempel segmented control whose selection is proven — by an
automated test written to fail against a conditional-rendering implementation — to survive a tab
round trip. Final automated numbers: root `npm test` 2 passed (backend + infrastructure,
unchanged from baseline); `app/npm test` 3 test suites / 10 tests passed (1 pre-existing +
3 Icon + 4 tab-navigation + 2 segment tests); `app/npm run lint` 0 errors, the same 1
pre-existing warning; `app/npx tsc --noEmit` exactly the same 1 pre-existing error. No regression
in any baseline number.

Walking issue #3's four acceptance criteria against the code (not a running app — see the
deviation below): (1) on launch, Karte is the active tab and `KarteScreen` is the visible slot —
proven by `TabNavigation.test.tsx`'s first test. (2) tapping Erfolge recolors its icon/label
(`TabBar`'s active/inactive class and `Icon` color props) and shows the Erfolge placeholder —
proven by the second and third tests. (3) returning to Karte preserves the segment selection —
proven concretely by the round-trip segment test; the status-filter half of this criterion is
deferred to issue #5 by design (see Decision Log), and inherits the same mounted-screen guarantee
for free once that issue adds a filter control. (4) the tab icons carry stroke-width 2.75, round
linecaps, and the exact vendored two-path `karte` data — proven directly by `Icon.test.tsx`
(`cd app && npm test -- Icon`).

**Deviation from the plan worth flagging clearly.** This execution ran in an environment without a
usable CocoaPods installation and without an iOS simulator or Android emulator attached. Two
consequences: `bundle exec pod install` in `app/ios` could not be run (recorded in Surprises &
Discoveries under Milestone 1; `app/ios/Podfile.lock` is therefore unchanged and NOT part of any
commit in this plan — it must be regenerated by running `pod install` locally before an iOS build),
and the plan's "real acceptance" manual walkthroughs for Milestones 2 and 3 (visually comparing a
running app against `screen_map_v1.png` / `screen_rewards_v1.png`, tapping through tabs and the
segmented control by hand) were not performed. Every piece of automated acceptance this plan
specifies passed exactly as predicted, and the components were built and reviewed strictly against
the vendored prototype markup/paths and `app/DESIGN.md`'s token values rather than guessed, which
gives reasonable confidence — but the developer should run `cd app && npm run ios` (after `pod
install`) or `npm run android` and walk the manual acceptance checklist in Milestone 2 and
Milestone 3 before considering issue #3 fully closed.

**Other departures from the letter of the plan, all minor:** the code-quality review during
Milestone 1 and Milestone 3 each surfaced one finding that was overridden rather than fixed,
because fixing it would have contradicted this plan's own explicit contracts or reasoning (see the
two corresponding Decision Log entries dated 2026-08-21, executor). A test-only Jest manual mock
for `react-native-safe-area-context` (`app/__mocks__/react-native-safe-area-context.js`) was added
in Milestone 2 that the plan's Interfaces and Dependencies file list did not anticipate; it was
necessary for `SafeAreaProvider` to render its children under `react-test-renderer` and is
documented in Surprises & Discoveries. No other files outside the plan's allowed list were
touched, and the root `package.json`/`package-lock.json` were never modified.
